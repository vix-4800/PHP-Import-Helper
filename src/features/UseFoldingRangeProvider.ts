...
export class UseFoldingRangeProvider implements vscode.FoldingRangeProvider {
    ...
    public provideFoldingRanges(document: vscode.TextDocument): vscode.FoldingRange[] {
        return this.calculator.calculate(document.getText().split(/\r?\n/))
            .map((range) => new vscode.FoldingRange(range.startLine, range.endLine, vscode.FoldingRangeKind.Imports));
    }
}
